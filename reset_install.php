...
<head>
    ...
    <title>BMCODEX - Reset i Instalacja</title>
    <style>
...
if (!isset($_GET['run'])) {
    ?>
        <div class="warning">
            ⚠️ UWAGA: Ten skrypt usunie wszystkie dane z bazy danych i zainstaluje sklep od nowa!
        </div>
        <div class="step">
            <h3>Reset i instalacja</h3>
            <p>Ten skrypt automatycznie:</p>
            <ul style="margin: 15px 0 15px 20px; color: #ccc;">
                <li>Usunie wszystkie istniejące tabele z bazy danych</li>
        ...
        <a href="?run=1" class="btn">Rozpocznij reset i instalację</a>
    <?php
    exit;
}
...
echo '<div class="step"><h3><span class="loading"></span>Reset i instalacja w toku...</h3></div>';
flush();
...
try {
    ...
    // Krok 1: Reset bazy danych (migrate:fresh)
    echo '<div class="step">';
    echo '<h3>🗑️ Usuwanie starych tabel i tworzenie nowych...</h3>';
    flush();
    ...
    if ($status === 0) {
        echo '<p style="color: #4CAF50;">✅ Baza danych zresetowana i migracje wykonane!</p>';
        if ($output) echo '<pre>' . htmlspecialchars($output) . '</pre>';
    } else {
        throw new Exception('Błąd podczas resetu bazy: ' . $output);
    }
    echo '</div>';
    flush();
